...
>
> **Terraform Keycloak provider** — provider ที่ดูแลโดย community อย่าง `keycloak/keycloak` (github.com/keycloak/terraform-provider-keycloak ซึ่งสืบทอดจาก `mrparkers/terraform-provider-keycloak` ที่เลิกดูแลแล้ว) เรียก Admin REST API เบื้องหลังด้วย service account credentials เพื่อสร้างและจัดการ realm, client และ role แบบ declarative
...
export const quizAdminApi = [
  ...
  },
  {
    ...
    options: [
      'สื่อสารกับ database โดยตรง',
      'cache admin token ไว้ในเครื่อง ทำให้ authenticate เพียงครั้งเดียวต่อ session',
      'ข้าม realm isolation',
      'ไม่ต้องใช้ Keycloak server ที่กำลังทำงานอยู่',
    ],
    answer: 1,
  },
];
