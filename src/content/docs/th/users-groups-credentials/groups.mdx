...
>
> **ทีม engineering ที่แบ่งเป็น subgroup** — ใช้โครงสร้าง `engineering` → `backend`, `frontend` เพื่อ map role เฉพาะทาง (เช่น `deploy-backend`) ให้เฉพาะ subgroup ที่เกี่ยวข้อง เวลามีคนย้ายทีมก็แค่ย้าย membership ระหว่าง subgroup ไม่ต้องแก้ role assignment ใหม่ทั้งหมด
