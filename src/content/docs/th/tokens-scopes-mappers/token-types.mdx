...
- อายุสั้นโดยค่าเริ่มต้น (5 นาทีใน Keycloak)
- บรรจุ claim เกี่ยวกับผู้ใช้และสิทธิ์ที่ได้รับ (role, scope)
- มีไว้สำหรับ API ของคุณ — ไม่ใช่สำหรับตัว client application เอง
- ควร validate ทุก request ด้วย public key ของ realm (ผ่าน JWKS)
...
- บรรจุ identity claim: `sub`, `name`, `email`, `preferred_username` และอื่นๆ
- มีไว้สำหรับ client application เท่านั้น — **ห้ามส่งไปยัง API ของคุณ**
- ไม่ควรใช้เพื่อตัดสินใจเรื่อง authorization
...
- ส่งได้เฉพาะไปยัง Keycloak token endpoint (`/protocol/openid-connect/token`)
- ไม่ส่งไปยัง API ของคุณเด็ดขาด
- ถูกยกเลิกเมื่อผู้ใช้ logout หรือเมื่อ SSO session หมดอายุ
- ถ้าขอ scope `offline_access` Keycloak จะออก offline refresh token ที่มีอายุอยู่ได้นานกว่า session ปกติ
