...
5. `quay.io/keycloak/keycloak:latest` — official Keycloak image จาก Red Hat Quay registry ควร pin version เฉพาะเจาะจง (เช่น `26.7`) ใน CI หรือ staging
...
- **รัน `start-dev` แล้วลืมสลับเป็น `start` ก่อนขึ้น production** — `start-dev` ปิด security check หลายตัวและใช้ in-memory database ถ้าหลงลืมสลับโหมด ข้อมูล production จะหายทันทีที่ container restart
